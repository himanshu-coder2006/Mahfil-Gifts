...
export default function Header() {
  ...
  return (
    <>
      ...
      <header
        className={`sticky top-0 z-40 border-b border-line/80 bg-white/95 backdrop-blur transition-shadow ${scrolled ? 'shadow-md' : 'shadow-none'}`}
        onMouseLeave={resetHover}
      >
        <div className="relative mx-auto flex h-[4.5rem] max-w-7xl items-center justify-between px-4 md:px-6">
          {/* Mobile hamburger */}
          ...
          <Link
            to="/"
            aria-label="Mahfil Gifts"
            className="absolute top-1/2 left-1/2 z-10 -translate-x-1/2 -translate-y-1/2 select-none md:-translate-y-[42%]"
            onClick={() => setMobileOpen(false)}
          >
            <img src="/mahfilgifts-teddy-logo.svg" alt="Mahfil Gifts" className="h-16 w-auto object-contain md:h-20 lg:h-[5.25rem]" />
          </Link>

          {/* Desktop nav */}
          <nav className="hidden flex-1 items-center gap-1 lg:flex lg:pr-40">
            {NAV.map((item) => {
              if (item.labelExtra) {
                return (
              ...
              return (
                <div key={item.label} className="relative" onMouseEnter={() => { clearTimeout(closeTimer.current); setOpenMenu(item.label); }}>
                  <button
                    className={`flex items-center gap-1 border-b-2 border-transparent px-3 py-2 text-sm font-medium transition hover:border-accent hover:text-accent ${openMenu === item.label ? 'border-accent text-accent' : 'text-primary'}`}
                  >
        ...
        <div className="fixed inset-0 z-[80] overflow-y-auto bg-primary/60 backdrop-blur-sm" onClick={closeSearch}>
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="font-display text-lg font-bold text-primary">Search MahfilGifts</h3>
        ...
        <div className="fixed inset-0 z-[80] bg-primary/50 backdrop-blur-sm" onClick={() => setMobileOpen(false)}>
          <div className="absolute top-0 left-0 bottom-0 w-[86%] max-w-sm overflow-y-auto bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 z-10 flex items-center justify-between bg-white px-5 py-4">
              <img src="/mahfilgifts-teddy-logo.svg" alt="Mahfil Gifts" className="h-16 w-auto object-contain" />
