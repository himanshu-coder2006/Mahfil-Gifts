      ...
      <div className="mx-auto max-w-7xl px-4 pt-16 pb-8 md:px-6">
        <div className="grid gap-10 md:grid-cols-4 lg:grid-cols-5">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="mt-2 text-sm text-white/65">Made for them. Made by you.</p>
            ...
            <div className="mt-5 space-y-2 text-sm text-white/70">
              <p className="flex items-center gap-2"><Mail className="h-4 w-4 text-accent" /> hello@mahfilgifts.com</p>
            ...
            <div className="mt-5 flex gap-3">
              {[{ Icon: Facebook, label: 'Facebook' }, { Icon: Instagram, label: 'Instagram' }, { Icon: Youtube, label: 'YouTube' }, { Icon: Twitter, label: 'Twitter' }].map(({ Icon, label }) => (
                <a key={label} href="/" aria-label={label} className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-white/75 transition hover:bg-accent hover:text-white">
          ...
          {COLS.map((col) => (
            <div key={col.title}>
              <p className="mb-4 text-[11px] font-semibold tracking-[0.2em] text-accent uppercase">{col.title}</p>
              <ul className="space-y-2.5">
                {col.links.map(([label, href]) => (
                  <li key={label}>
                    <Link to={href} className="text-sm text-white/70 transition hover:text-accent">{label}</Link>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="lg:col-span-1">
            <p className="mb-4 text-[11px] font-semibold tracking-[0.2em] text-accent uppercase">Newsletter</p>
            ...
            <form onSubmit={subscribe} className="mt-4 flex flex-col gap-2 sm:flex-row">
              <input
                ...
                className="w-full rounded-full border border-white/20 bg-white/10 px-4 py-2.5 text-sm text-white outline-none placeholder:text-white/45 focus:border-accent"
              />
              <button type="submit" className="shrink-0 rounded-full bg-accent px-5 py-2.5 text-sm font-semibold transition hover:bg-accent-dark">
                Subscribe
              </button>
            </form>
          </div>
        ...
        <div className="mt-10 flex flex-col items-center justify-between gap-3 border-t border-white/10 pt-6 md:flex-row">
          <p className="text-xs text-white/50">© 2026 MahfilGifts. All rights reserved.</p>
